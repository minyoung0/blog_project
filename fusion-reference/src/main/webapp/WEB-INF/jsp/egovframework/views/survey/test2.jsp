...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /*   const questions = document.querySelectorAll(".question, .sub-question"); */
    ...
    const questions = document.querySelectorAll(".question"); // 대문항만 선택
    const subQuestions = document.querySelectorAll(".sub-question"); // 소문항 선택
   ...
   function showPage(page) {
       ...
       questions.forEach((question, index) => {
           ...
           subQuestions.forEach(subQuestion => {
        	    const parentId = subQuestion.getAttribute("data-parent-question-id");
        	    const subPage = parseInt(subQuestion.getAttribute("data-page"), 10);
        	    const isVisible = page === subPage && parentId === questionId;
               subQuestion.style.display = isVisible && parentId === questionId ? "block" : "none";
           });
       });
    ...
    document.getElementById("surveyForm").addEventListener("submit", function (event) {
     ...
     // Debugging: 콘솔 출력
        ...
        console.log("Response Created At:", responseCreateAt);
...
<body>
	...
	<h1>설문조사</h1>
	<form action="/survey/submitSurvey.do" method="POST" id="surveyForm" data-user-id="${userId}">
			...
			<c:forEach var="question" items="${questions}">
				...
				<c:if test="${question.parentQuestionId == 0}">
					...
					<div class="question" data-page="${page}" data-question-id="${question.questionId}" data-question-type="${question.questionType != null ? question.questionType : ''}"
						data-is-required="${question.isRequired}" data-question-number="${question.questionNumber} ">
						<p>
					...
					<c:forEach var="subQuestion" items="${questions}">
						<c:if test="${subQuestion.parentQuestionId == question.questionId}">
							<div class="sub-question nested" data-page="${page}" data-question-id="${subQuestion.questionId}" data-question-type="${subQuestion.questionType}" data-is-required="${subQuestion.isRequired}"
								data-question-number="${subQuestion.questionNumber}" data-parent-question-id="${subQuestion.parentQuestionId}">

								<strong>문항 ${subQuestion.questionNumber}.</strong> ${subQuestion.questionText}
								<!-- subQuestion의 questionId와 questionNumber 표시 -->
								<p>Question ID: ${subQuestion.questionId}</p>
								<p>Question Number: ${subQuestion.questionNumber}</p>
								<c:choose>
									...
									<c:when test="${subQuestion.questionType == 'multiple'}">
										<ul>
											<c:forEach var="choice" items="${subQuestion.choices}">
												<li><input type="checkbox" name="question_${subQuestion.questionId}" value="${choice.choiceId}"
													class="limited-checkbox ${choice.choiceText == '없음' ? 'none-option' : ''} ${choice.choiceText == '기타' ? 'etc-option' : ''}"> ${choice.choiceText} <input type="text"
													class="etc-textbox" style="display: none;" placeholder="기타 내용을 입력하세요"></li>
